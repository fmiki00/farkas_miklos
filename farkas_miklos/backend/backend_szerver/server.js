...
app.get("/regio_tipus", (req, res) => {
    const sql = "SELECT * FROM regiok WHERE regio_tipusa LIKE 'r%'";
    db.query(sql, (err, result) => {
        if (err) return res.json(err);
        return res.json(result)
    })
...
app.get("/regio_szam", (req, res) => {
    const sql = "SELECT COUNT(regionev) FROM `regiok` WHERE regio_tipusa LIKE 'r%'";
    db.query(sql, (err, result) => {
        if (err) return res.json(err);
        return res.json(result)
    })
